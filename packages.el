...
(use 'apache-mode)
(use 'apples-mode
  (defalias 'applescript-mode 'apples-mode)
  (setq apples-decompile-query ?o)
  (setq apples-decompile-callback (lambda (src file)
    (apples-handle-decompile src file)
    (save-excursion
      (goto-char (point-min))
      (insert "#!/usr/bin/env osascript -sh\n"))))
  (add-hook 'apples-mode-hook (lambda()
    (setq apples-continuation-offset 4)
    (setq apples-indent-offset 4)
    (configure-indent)))
  (add-to-list 'auto-mode-alist '("\\.\\(applescri\\|sc\\)pt\\'" . apples-mode))
  (add-to-list 'interpreter-mode-alist '("osascript" . (lambda()
    (save-excursion
      (goto-char (point-min))
      (if-let* (((looking-at auto-mode-interpreter-regexp))
                (mode (match-string 2))
                ((string= mode "osascript"))
                ((goto-char (+ 9 (match-beginning 2))))
                ((looking-at "[ \t]+[^\n]*[ \t]-l[ \t]*JavaScript\\>")))
        (js2-mode)
        (apples-mode))))))
  (add-to-list 'magic-mode-alist '("^FasdUAS" . (lambda()
    (apples-mode)
...
(use 'ebnf-mode)
(use 'editorconfig)
(use 'editorconfig-domain-specific)
(use 'editorconfig-custom-majormode)
(use 'editorconfig-charset-extras)
(use 'edit-server
